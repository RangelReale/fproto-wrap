...
func (g *GeneratorFile) FileDep(filedep *fdep.FileDep, defalias string, is_gowrap bool) string {
	...
	var p string
	if is_gowrap && !filedep.IsSamePackage(g.G().GetFileDep()) && g.G().IsFileWrap(filedep) {
...
func (g *GeneratorFile) Filename() string {
	p := g.G().GoWrapPackage(g.G().GetFileDep())
	return path.Join(p, strings.TrimSuffix(path.Base(g.G().GetFileDep().FilePath), path.Ext(g.G().GetFileDep().FilePath))+g.Suffix+".fwpb.go")
}

func (g *GeneratorFile) generateHeader() {
	p := fproto_wrap.BaseName(g.G().GoWrapPackage(g.G().GetFileDep()))

	g.P("// Code generated by fproto-gowrap. DO NOT EDIT.")
	g.P("// source file: ", g.G().GetFileDep().FilePath)
