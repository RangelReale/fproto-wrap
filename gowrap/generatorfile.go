...
func (g *GeneratorFile) GetDeclFileDep(depfile *fdep.DepFile, defalias string, is_gowrap bool) (retimp string, retdefalias string) {
	if depfile == nil {
		depfile = g.G().GetDepFile()
	}
	if g.G().IsFileWrap(depfile) {
		if g.G().GetDepFile() != nil {
			if is_gowrap {
				defalias = g.G().GoWrapFilePackage(g.G().GetDepFile())
			} else {
				defalias = g.G().GoFilePackage(g.G().GetDepFile())
			}
		} else {
			if is_gowrap {
				defalias = g.G().GoWrapFilePackage(depfile)
			} else {
				defalias = g.G().GoFilePackage(depfile)
			}
		}
	}
	var p string
...
func (g *GeneratorFile) DeclFileDep(depfile *fdep.DepFile, defalias string, is_gowrap bool) string {
	dimp, ddefalias := g.GetDeclFileDep(depfile, defalias, is_gowrap)
	return g.DeclDep(dimp, ddefalias)
}
...
func (g *GeneratorFile) generateHeader() {
	var p string
	if g.FixedPackageName != "" {
		p = g.FixedPackageName
	} else {
		p = g.G().GoWrapFilePackage(g.G().GetDepFile())
	}
